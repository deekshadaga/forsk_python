...
d7=df['mean'] = df.groupby('service')['salary'].transform('mean')
print(d7)

#4. Missing phd - should be mean of the matching service
d6=df['mean'] = df.groupby('service')['phd'].transform('mean')
print(d6)
...
d4=df[df.service == df.service.max()]
d5=df[df.service == df.service.min()]
print('For employees :\nSenior most is =\n',d4,'\nJunior most are =\n',d5)

#8. Draw a histogram of the salaries divided into bin starting from 50K and increment of 15K
plt.hist(df['salary'], bins = range(50000, int(df.salary.max()), 15000))
plt.show()